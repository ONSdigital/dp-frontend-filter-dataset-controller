			...
			for _, opt := range opts {
				if opt.Option == hv.Links.Self.ID {
					if _, ok := req.Form[hv.Links.Self.ID]; !ok {
						if err := f.FilterClient.RemoveDimensionValue(req.Context(), userAccessToken, f.serviceAuthToken, collectionID, filterID, name, hv.Links.Self.ID); err != nil {
							log.ErrorCtx(ctx, err, nil)
						}
					}
				}
...
func (f *Filter) addAllHierarchyLevel(w http.ResponseWriter, req *http.Request, fil filter.Model, name, code, redirectURI string) {
	...
	if err := f.FilterClient.AddDimensionValues(req.Context(), userAccessToken, f.serviceAuthToken, collectionID, fil.FilterID, name, options); err != nil {
		log.ErrorCtx(ctx, err, nil)
	}
...
func (f *Filter) removeAllHierarchyLevel(w http.ResponseWriter, req *http.Request, fil filter.Model, name, code, redirectURI string) {
	...
	for _, child := range h.Children {
		if err := f.FilterClient.RemoveDimensionValue(req.Context(), userAccessToken, f.serviceAuthToken, collectionID, fil.FilterID, name, child.Links.Self.ID); err != nil {
			log.ErrorCtx(ctx, err, nil)
		}
	}
