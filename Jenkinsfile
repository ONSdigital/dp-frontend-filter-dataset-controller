...
node {
    ...
    ws("${gopath}/src/github.com/ONSdigital/dp-frontend-dataset-controller") {
        stage('Checkout') {
            checkout scm
            sh 'git clean -dfx'
        ...
        stage('Image') {
            docker.withRegistry("https://${env.ECR_REPOSITORY_URI}", { ->
                docker.build('frontend-dataset-controller', '--no-cache --pull --rm .').push(revision)
            })
        }
        ...
        stage('Bundle') {
            ...
            sh "tar -cvzf frontend-dataset-controller-${revision}.tar.gz appspec.yml scripts/codedeploy"
            sh "aws s3 cp frontend-dataset-controller-${revision}.tar.gz s3://${env.S3_REVISIONS_BUCKET}/"
        }
        ...
        stage('Deploy') {
            def appName = 'frontend-dataset-controller'
